...
on_ground = (place_meeting(x, y + 1, obj_ground_hitbox) && y_vel = 0);
...
if (place_meeting(x + x_vel, y, obj_ground_hitbox)){
	x_dir = sign(x_vel);
	while (!place_meeting(x + x_dir, y, obj_ground_hitbox)) {
		x += x_dir;
	}
	x_vel = 0;
}
...
//vertical collision
if (place_meeting(x,y + y_vel, obj_ground_hitbox)){
	y_dir = sign(y_vel);
	while (!place_meeting(x, y + y_dir, obj_ground_hitbox)) {
		y += y_dir;
	}
	y_vel = 0;
}
